...
export const SessionStepperView: React.FC<SessionStepperViewProps> = ({
  ...
  const handleExportPracticePdf = async () => {
    ...
    try {
      await generateOfficialPdf(lastSavedStintLap, module, session, lastSavedStint || undefined);
    } catch (err) {
      console.error('Failed to generate practice PDF:', err);
    } finally {
      setIsGeneratingPracticePdf(false);
    }
  };

  // Challenge Attempt PDF Export (Step 5)
  const handleExportChallengePdf = async (attempt: ChallengeAttempt) => {
    if (!attempt || !attempt.laps || attempt.laps.length === 0 || generatingAttemptId) return;
    setGeneratingAttemptId(attempt.id);
    try {
      const bestLap = attempt.laps.reduce(
        (best, l) => (best.lapTimeSec < l.lapTimeSec ? best : l),
        attempt.laps[0]
      );
      const challengeStint: StintSession = {
        stintId: attempt.id,
        stintNumber: attempt.attemptNumber,
        title: `${session.title} (Challenge Attempt #${attempt.attemptNumber})`,
        carName: 'Formula Skip Barber 2000',
        trackName: 'Lime Rock Park - Full Circuit',
        source: 'academy',
        recordedAt: attempt.timestamp,
        durationSec: Math.round(attempt.laps.reduce((sum, l) => sum + l.lapTimeSec, 0)),
        totalLaps: attempt.laps.length,
        bestLapTimeSec: bestLap.lapTimeSec,
        avgScore: Math.round(attempt.laps.reduce((sum, l) => sum + (l.overallScore || 0), 0) / attempt.laps.length),
        laps: attempt.laps,
        moduleNumber: module.moduleNumber,
        moduleTitle: module.title,
        sessionId: session.id,
      ...
      await generateOfficialPdf(bestLap, module, session, challengeStint, null, attempt);
    } catch (err) {
      console.error('Failed to generate challenge attempt PDF:', err);
    } finally {
                ...
                <div className="p-4 bg-[#101018] border border-emerald-900/40 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    <button
                      ...
                      disabled={isGeneratingPracticePdf}
                      className="chamfer-btn flex items-center space-x-2 px-4 py-2.5 bg-[#181826] hover:bg-[#222234] text-white text-xs font-racing font-bold tracking-wide border border-[#303048] shadow-lg shadow-black/40 active:scale-95 cursor-pointer disabled:opacity-60 disabled:cursor-wait transition-all"
                    >
                      {isGeneratingPracticePdf ? (
                        <Loader2 className="w-4 h-4 animate-spin text-amber-400" />
                      ) : (
                        <FileDown className="w-4 h-4 text-[#00F0FF]" />
                      )}
                      <span>{isGeneratingPracticePdf ? 'Generating PDF...' : 'Export Practice Debrief PDF'}</span>
                    </button>
              ...
              {selectedAttempt && (
                <div className={`p-5 border ${
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => handleExportChallengePdf(selectedAttempt)}
                        disabled={generatingAttemptId === selectedAttempt.id}
                        className="chamfer-btn flex items-center space-x-1.5 px-3.5 py-1.5 bg-[#181826] hover:bg-[#222236] text-slate-100 hover:text-white text-xs font-mono border border-[#34344E] cursor-pointer disabled:opacity-50 transition-all shadow-md"
                        title="Export Official Challenge Dossier PDF"
                      >
                        {generatingAttemptId === selectedAttempt.id ? (
                          <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-400" />
                        ) : (
                          <FileDown className="w-3.5 h-3.5 text-amber-400" />
                        )}
                        <span>{generatingAttemptId === selectedAttempt.id ? 'Generating PDF...' : 'Export Challenge PDF'}</span>
                      </button>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    {selectedAttempt && (
                      <button
                        onClick={() => handleExportChallengePdf(selectedAttempt)}
                        disabled={generatingAttemptId === selectedAttempt.id}
                        className="chamfer-btn flex items-center space-x-1 px-2.5 py-1 bg-[#181826] hover:bg-[#242438] text-amber-300 hover:text-amber-200 border border-[#3A3520] text-xs font-mono cursor-pointer disabled:opacity-50 transition-all"
                        title="Export Official Challenge Dossier PDF"
                      >
                        {generatingAttemptId === selectedAttempt.id ? (
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-3 text-right space-x-2">
                              <button
                                onClick={() => handleExportChallengePdf(att)}
                                disabled={generatingAttemptId === att.id}
                                className="px-2.5 py-1 bg-[#181824] hover:bg-[#252538] text-amber-300 hover:text-amber-200 border border-[#3A3520] text-[11px] font-mono transition-all cursor-pointer inline-flex items-center space-x-1 disabled:opacity-50"
                                title="Export Certified Challenge Dossier PDF"
                              >
                                {generatingAttemptId === att.id ? (
