    ...
    <div className="flex-1 overflow-y-auto p-8 bg-[#0A0A0E] space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-[#12121A] border border-[#232332] flex items-center space-x-4 shadow-lg">
          ...
          <div>
            <span className="text-xs text-[#8E8E9F] font-mono block">Certified Modules</span>
            <strong className="text-2xl font-display font-black text-white">{graduatedCount} <span className="text-xs text-slate-500 font-normal">/ {modules.length}</span></strong>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12121A] border border-[#232332] flex items-center space-x-4 shadow-lg">
          ...
          <div>
            <span className="text-xs text-[#8E8E9F] font-mono block">Passed Sessions</span>
            <strong className="text-2xl font-display font-black text-white">{completedSessionsCount}</strong>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12121A] border border-[#232332] flex items-center space-x-4 shadow-lg">
          ...
          <div>
            <span className="text-xs text-[#8E8E9F] font-mono block">Total Telemetry Laps</span>
            <strong className="text-2xl font-display font-black text-white">{progress.totalLapsDriven + savedLaps.length}</strong>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12121A] border border-[#232332] flex items-center space-x-4 shadow-lg">
          ...
          <div>
            <span className="text-xs text-[#8E8E9F] font-mono block">Academy Rank</span>
            <strong className="text-lg font-display font-black text-purple-300">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-display font-bold uppercase tracking-wider text-white flex items-center space-x-2">
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs shrink-0 ${
                  isGrad ? 'bg-amber-400 text-black' : 'bg-[#1C1C26] text-slate-500'
                }`}>
      ...
      <div className="space-y-3">
        ...
        {savedLaps.length === 0 ? (
          <div className="p-8 rounded-2xl bg-[#12121A] border border-[#20202E] text-center text-xs text-slate-400">
            ...
            {savedLaps.map((lap, idx) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className="w-8 h-8 rounded-lg bg-[#E10600]/20 text-[#FF4D4D] font-mono font-bold flex items-center justify-center text-xs border border-[#E10600]/40">
                    #{lap.lapNumber}
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white">Watkins Glen Practice Lap</h4>
                    <span className="text-[10px] font-mono text-[#8E8E9F]">
                      Traction Budget: {lap.avgTractionBudgetPct}% • Top Speed: {lap.maxSpeedKph} km/h
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-6 text-xs font-mono">
                  <div>
                    ...
                    <strong className="text-emerald-400 font-bold text-sm">{lap.lapTimeSec.toFixed(2)}s</strong>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 block">Overall Score</span>
                    <strong className="text-amber-400 font-bold text-sm">{lap.overallScore}%</strong>
                  </div>
                  <button
                    onClick={() => onSelectLapForDebrief(lap)}
                    className="px-3 py-1.5 rounded-lg bg-[#1F1F2C] hover:bg-[#E10600] text-slate-200 hover:text-white text-xs font-semibold border border-[#2D2D3E] transition-all"
                  >
                    Open Debrief
                  </button>
                </div>
